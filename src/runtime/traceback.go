...
/*
此文件中的代码实现了所有架构的堆栈跟踪行走。给定体系结构最重要的事实是它是否使用连接寄存器。
在具体有连接寄存器的系统上，non-leaf 函数的序言将LR的传入值存储在新分配的堆栈帧的底部。
在没有连接寄存器(x86)的系统上，体系结构在调用指令期间推送返回PC，因此返回PC结束于堆栈帧上。
在这个文件中，返回的PC总是被称为LR，无论它是如何找到的。
*/
