...
gopark作用：
	...
	3.获取一个新goroutine来运行 gopark函数的关键就是mcall函数调用park_m
*/
...
/*
与gopark相反的，goready的作用就是唤醒waiting状态的goroutine
*/
func goready(gp *g, traceskip int) {
	// 通过 systemstack切到g0栈，在g0栈上发起调度。
	systemstack(func() {
...
func ready(gp *g, traceskip int, next bool) {
	...
	// 状态从waiting或者scanwaiting转成 runnable，和加到runq队列
...
//go:yeswritebarrierrec
/*
计划gp在当前M上运行。如果inheritTime为true，gp将继承当前时间片中的剩余时间。
否则，它将启动一个新的时间片。永远不回头。
写入障碍是允许的，因为在多个地方获取p后，立即调用该障碍
*/
...
// Never returns.
/*
所有状态为Grunnable的任务都可能通过findrunnable函数被调度器(P&M)获取，进而通过execute函数
将其状态切换到Grunning，最后调用gogo()加载其上下文并执行。
*/
func schedule() {
	_g_ := getg()
...
3.在g0栈上，调用park_m
4.将当前的g从running状态设置为waiting状态
5.通过dropg来解除m和g的关系。
6.最后通过scheduule来发起新一轮的调度 schedule() -> execute() ->gogo()，
	gogo尝试从gobuf中恢复出协程执行状态并跳转到上一次指令处继续执行。
*/
/*任务状态变动
当一个任务需要的资源或运行条件不能被满足时，需要调用gopark函数进入该状态，之后除非等待条件满足，
否则任务将一直处于等待状态不能执行。(像channel，定时器、网络IO操作都可能引起任务的阻塞)
...
//
/*
主要核心：将M从P剥离出去，通知调度器，P已经放弃M，需要调度其他G
	但M仍然记着P，在系统调度返回后，M还尽可能找回原来的P，至于P有没有另结新欢就看情况
注意：P放弃了M，但是还没有给其他G找到另一个M，只是将P的状态标记为Psyscall
*/
//go:nosplit
func reentersyscall(pc, sp uintptr) {
	...
	// 禁用抢占，因为在此函数期间，g处于Gsyscall状态，但可能具备不一致的g.sched，不要让GC观察它
	_g_.m.locks++
...
/*
go系统调用库和正常cgo调用使用的标准系统调用条目
这通过链接名导出到syscall包和x/sys中的程序集
*/
//
//go:nosplit
//go:linkname entersyscall
/*
系统调用是一个相对耗时的过程。一旦P中的某个G进入系统调用状态而阻塞了该P内的其他协程。
此调度器必须得做点什么，这就是调度器在进入系统调用前call的目的所在。
*/
func entersyscall() {
...
//go:nosplit
/*
Go运行时为了保证高的并发性能，当会在任务执行OS系统调用前，先调用entersyscall函数将自己的状态置为Gsyscall
如果系统调用是阻塞式的或者执行过久，则将当前M与P分离
当系统调用返回后，执行线程调用 exitsyscall尝试重新获取P，如果成功且当前任务没有被抢占，
则将状态切回Grunning并继续执行；否则将状态置为Grunnable，等待再次被调度执行。
*/
...
//
/*
goroutine g退出了它的系统调用。安排它再次在cpu上运行。这只从go系统调用库调用，
而不是从运行时使用的低级系统调用。写障碍是不允许的，因为我们的P可能被窃取了。
这将通过linknae导出到syscall包中的程序集.
*/
/* M从系统调用返回的过程
1.首先是否能回到原来的P
2.如果原来的P不在是Psyscall状态，则随便选一个P
3.如果1,2（1,2就是exitsyscallfast的主要流程）都找不到，只能将g放入全局P，进入闲置状态。
*/
//go:nosplit
//go:nowritebarrierrec
...
func exitsyscall() {
	...
	if exitsyscallfast(oldp) {
		...
		// write barriers
		// 当exitsyscallfast返回成功时，我们有一个P，因此现在可以使用写屏障
		if goroutineProfile.active {
			// Make sure that gp has had its stack written out to the goroutine
			// profile, exactly as it was when the goroutine profiler first
			// stopped the world.
			// 确保gp已将其堆栈写入goroutine配置文件中，与goroutine探查器第一次停止世界时完全相同
			systemstack(func() {
		...
		// 垃圾收集器没有运行(因为我们正在运行)，所以可以清除 syscallsp
		_g_.syscallsp = 0
		_g_.m.locks--
		if _g_.preempt {
			...
			// 否则，恢复真实的 _StackGuard，我们在entersyscall/entersyscallblock中破坏了它
			_g_.stackguard0 = _g_.stack.lo + _StackGuard
		}
	...
	if trace.enabled {
		...
		// 等待直到发出 traceGoSysBlock事件，这确保了跟踪的一致性(goroutine在被阻止后启动)
	...
	// Call the scheduler.
	// 如果M回来发现P已有其他M服务，那只能将自己挂起，等待其他P服务
	mcall(exitsyscall0)
	...
	// is not running.
	/*
		调度程序返回，因此现在允许我们运行。删除系统调度期间留给垃圾收集器的syscallsp信息必须等到现在，
		因为在gosched返回之前，我们无法确认垃圾收集器是否正在运行。
	*/
	_g_.syscallsp = 0
	_g_.m.p.ptr().syscalltick++
	_g_.throwsplit = false
...
func exitsyscallfast(oldp *p) bool {
	...
	// Freezetheworld 设置了 stopwait，但没有重夺P
		...
		systemstack(func() {
			...
			if ok && trace.enabled {
				if oldp != nil {
					// Wait till traceGoSysBlock event is emitted.
					// This ensures consistency of the trace (the goroutine is started after it is blocked).
					// 一直等待发出 traceGoSysBlock事件。这确保了跟踪的一致性(goroutine在被阻止后启动)
...
//
/*
退出g0上的慢路径。未能获取P，将gp排队为可运行。
通过mcall调用，因此gp是来自次M的调用g
*/
//go:nowritebarrierrec
func exitsyscall0(gp *g) {
	...
	if _p_ == nil {
		...
		// locked.
		/*
			下面，如果gp被锁定，我们执行 stoplockedm. globrunqput释放gp的所有权。
			因此我们必须在提交释放之前通过解锁 sched.lock，检查gp是否被锁定，
			否则我们可能会与另一个M将gp从解锁转为锁定
		*/
		locked = gp.lockedm != 0
	...
	if locked {
		...
		// before entersyscall.
		//等待另一个线程再次调度gp和m
		// 注意：lockedm这个必须是这个M，因为在entersyscall之前，这个g在这个M上运行
		stoplockedm()
		execute(gp, false) // Never returns.
	}
	stopm()
...
func malg(stacksize int32) *g {
	...
	if stacksize >= 0 {
		...
		// 设置 stackguard，在协程栈不够用时再重新申请新的栈
...
/*
创建一个新的 运行的g函数，把他放在等待运行的g队列中，编译器将go语法转换为对此的调用
*/
func newproc(fn *funcval) {
	...
	// 切到g0栈
	systemstack(func() {
		// 执行newproc1(goroutine诞生的地方)
		newg := newproc1(fn, gp, pc)

		_p_ := getg().m.p.ptr()
		// 将g加入队列p(本地或者全局)
		runqput(_p_, newg, true)
		// 尝试唤醒一个真正的p来执行g，p的空闲队列中获取一个
		if mainStarted {
			wakep()
		}
	})
...
/*
从fn开始，在状态Grunable中创建一个新的g。callerpc是创建此的go语法的地址
调用者负责将新的g添加到调度器
当newg完成资源的分配及初始化后，newg的状态会被置为Grunnable,然后被添加到
当前P的私有任务队列中，等待调度执行。
*/
func newproc1(fn *funcval, callergp *g, callerpc uintptr) *g {
	...
	// 获取本地的p
	_p_ := _g_.m.p.ptr()
	// 尝试从本地p的gfree上获取一个不用的g，或者从全局的p中获取
	newg := gfget(_p_)
	// 没有获取g的时候就去创建一个g，大小为2k
	if newg == nil {
		// 创建协程栈
		newg = malg(_StackMin)
		// 新的g状态为dead
		casgstatus(newg, _Gidle, _Gdead)
		// 加入全局allg的队列中 => 发布时的g状态为Gdead，因为GC扫描程序不会查看未初始化的堆栈
	...
	// 初始化g的一些参数
	newg.sched.sp = sp // 设置新建协程的栈顶sp
	newg.stktopsp = sp
	// +PCQuantum 以便前一条指令具有相同的功能
	...
	if isSystemGoroutine(newg, false) {
		...
		if goroutineProfile.active {
			...
			// _Gdead.
			/*
				并发goroutine配置文件正在运行。它应该包括在goroutine探查器
				第一次停止时仍然存在的一组goroutine。
				这不包括 新g，所以在将其从 dead转换之前，将其标志为不需要配置文件
			*/
	...
	if raceenabled {
		...
		if newg.labels != nil {
			...
			/*
				请参考专业标签中的注释。了解labelSync在与信号处理程序中的读取同步中的作用。
			*/
...
/*
从gfree列表中获取。如果本地队列为空，则从全局队列中获取一批
*/
func gfget(_p_ *p) *g {
retry:
...
//
// 将p与当前m相关联。即使调用方没有写屏障，也允许此函数具有写屏障，因为它会立即获取 _p_
//
//go:yeswritebarrierrec
func acquirep(_p_ *p) {
...
// Always runs without a P, so write barriers are not allowed.
// 始终在没有P的情况下运行，因此不允许写入障碍
//
/*
当系统调用M时，P被迫剥离M，导致P状态为Psyscall，只能等待陷入系统调用的m返回(这种情况不被允许)
sysmon线程，这是一个特殊的m，专门监控系统状态
sysmon周期性醒来，并且遍历所有的p，如果发现有Psyscall状态的p并且已经处于改状态超过一定时间了，
就再次安排一个m，这样p内的任务又可以得到处理。
*/
//go:nowritebarrierrec
func sysmon() {
	lock(&sched.lock)
...
/*
	runqput尝试将g放入本地可运行队列。
	如果next为false，runqput将g添加到可运行队列的尾部
	如果next为true，runqput将g放入p.runnext插槽。
