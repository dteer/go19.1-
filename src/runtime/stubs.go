...
/*
getg返回当前g的指针。编译器将对该函数的调用重写为直接(从TLS或专用寄存器)获取g的指令。
*/
func getg() *g
...
并在当前线程的g0协程堆栈上执行新的函数。一般情况下，会在新函数中执行一次schedule()
来挑选新的协程来运行。
*/
...
//go:noescape
/*
systemstack在系统堆栈上运行fn。如果从每个os线程(g0)堆栈调用systemstack，
或者从信号处理(gsignal)堆栈调用系统堆栈，则系统堆栈直接调用fn并返回。
否则，将从有限堆栈调用systemstack。
在这种情况下，systemstack切换到每个os线程堆栈，调用fn，然后切换回来。
通常使用func作为参数，以便与其他代码的调用共享输入和输出
